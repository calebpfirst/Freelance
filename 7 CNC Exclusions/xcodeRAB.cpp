...
/* CONSTANT VALUES */
const char* OUTPUT_FILENAME = "output"; 			// change this to whatever you would like the output filename to be.
...
string RemoveExt(string infilename)
{
	string output = "";
...
string GetExt(string infilename)
{
	string output = "";
	bool foundstart = false;
	for (int i = 0; i < infilename.size(); ++i)
...
int main()
{
	...
	cout << "Saving file...\n";
	string outputfile = RemoveExt(input);
	outputfile += '_';
	outputfile += OUTPUT_FILENAME;
	outputfile += '.';
	outputfile += GetExt(input);
	SaveOutputFile(outputfile.c_str());
	cout << "...done\n";
	getchar();
			...
			else if (hasLeadout)
			{
				...
				if (line.hasz && (line.z != 0.0))
				{
					...
					else
					{
						...
						{
							line.z = RoundToThree(line.z + (Zorg * ((Yorg - (Yp - Ys)) / Yorg) * ((Yorg - (Yp - Ys)) / Yorg)));
						}
					}
				}
			...
			else if (hasLeadin)
			{
				...
				if (line.hasz && (line.z != 0.0))
				{
					...
					else
					{
						...
						{
							line.z = RoundToThree(line.z + (Zorg * ((Yorg - (Ys - Yp)) / Yorg) * ((Yorg - (Ys - Yp)) / Yorg)));
						}
